...
def find_students(term):
    ...
    for student in student_db:
        if term.upper() in student.name.upper():
    ...
    if results == []:
        ...
        for student in results:
            print(f" Name: {student.name}, ID: {student.id}, Instrument:{student.enrolled_in}")
...
def find_teachers(term):
    ...
    for teacher in teacher_db:
        if term.upper() in teacher.name.upper() or term.upper() in teacher.speciality.upper():
